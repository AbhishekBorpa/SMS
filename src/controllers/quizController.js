...
const getQuizzesAvailable = asyncHandler(async (req, res) => {
    // If student, show quizzes for their class or public quizzes
    // If teacher, show quizzes they created
    ...
    if (req.user.role === 'Student') {
        ...
        const studentClass = student.className; // Assuming className is stored on User or we fetch connection

        query.$or = [
            { isPublic: true },
            { classIds: studentClass }
        ];
...
const submitQuiz = asyncHandler(async (req, res) => {
    ...
    if (linkedAssignment) {
        ...
        // We need the student's class ID. Assuming req.user has it or we fetch it.
        ...
        let classId = req.user.class; // If User model has ObjectId ref
        if (!classId && req.user.className) {
            const ClassModel = require('../models/Class');
            const classObj = await ClassModel.findOne({ name: req.user.className, school: req.schoolId });
            if (classObj) classId = classObj._id;
        }
